...
static const CGFloat kMaxVelocity = 550;
...
//        self.anchorPoint = CGPointMake(0.5,0.5);
        ...
        _ground = [Ground spriteNodeWithColor:[SKColor colorWithRed:.87 green:.84 blue:.59 alpha:1] size:CGSizeMake(self.size.width, 32)];
...
- (void)setupPlayer
{
    ...
    [_player setPosition:CGPointMake(0, self.size.height/2+200)];
    ...
    // Move top pipe
    SKAction *moveBird = [SKAction moveToX:self.size.width/2 duration:kPipeSpeed/2];
//    SKAction *pipeTopSequence = [SKAction sequence:@[pipeTopAction, [SKAction runBlock:^{
//        [pipeTop removeFromParent];
//    }]]];
    
    [_player runAction:moveBird];
...
//    CGFloat pipeBottomHeight = self.size.height - (centerY + (kPipeGap/2) * 2);
    CGFloat pipeBottomHeight = randomFloat(60, 120);
    ...
    pipeTop.size = CGSizeMake(randomFloat(40, 90), 20);
    
    [pipeTop setPosition:CGPointMake(self.size.width+(pipeTop.size.width/2), 200)];
    ...
    Obstacle* pipeBottom = [Obstacle spriteNodeWithImageNamed:@"pipe-bottom"];
     ...
     CGFloat pipeBottomWidth = randomFloat(20, 70);
    
    pipeBottom.size = CGSizeMake(pipeBottomWidth, pipeBottomHeight);
    ...
    // Move top pipe
    SKAction *pipeTopAction = [SKAction moveToX:-(250) duration:kPipeSpeed];
    SKAction *pipeTopSequence = [SKAction sequence:@[pipeTopAction, [SKAction runBlock:^{
        [pipeTop removeFromParent];
    }]]];
    ...
    // Move bottom pipe
    SKAction *pipeBottomAction = [SKAction moveToX:-(250) duration:kPipeSpeed];
...
//    }
    if (([nodeA isKindOfClass:[Player class]] && [nodeB isKindOfClass:[Ground class]]) ||
        ([nodeB isKindOfClass:[Player class]] && [nodeA isKindOfClass:[Ground class]]))
    {
//        if(ABS(nodeA.position.y - nodeB.position.y) < canshu){
//            
//        }
        _canJump = YES;
    }
    if (([nodeA isKindOfClass:[Player class]] && [nodeB isKindOfClass:[Obstacle class]]) ||
        ([nodeB isKindOfClass:[Player class]] && [nodeA isKindOfClass:[Obstacle class]]))
    {
        NSLog(@"nodeA.y:%f,nodeB.y:%f,canshu:%f",nodeA.position.y , nodeB.position.y , 20.);
        
        if(nodeA.position.y - nodeB.position.y < 25){
